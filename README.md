...
- Производительность модели:
  - Эффектвность CPU (CPU Utilization): Как было описано выше, все вычисление и работа модели производятся на процессоре AMD Ryzen 7 5800X - в момент запуска и работы модели потребление ресурсов процессора не превышает 10% от общих мощностей. Однако во время обучения модели нагрузка на процессор переодически в пике возрастала до 60%. При переходе на GPU и оптимизации запросов есть возможность в разы увеличить скорость работы и обучения модели, а также снизить общую нагрузку на систему.
  - Использование памяти (Memory Usage): В соответствии с небольшими объемами данных, которые были использованы в рамках проекта модели потребляет **до 3 Гб оперативной памяти** во время подбора резюме или вакансии.
  - Задержка (Latency): В рамках демоверсии проекта и использованного оборудования время между отправкой запроса и получением ответа от модели составляет **~4 секунды**.
